/** Persist and query the per-provider sync manifest — the three-way merge base mapping relative paths to their last-synced state. @depends syncTypes, remoteProvider. @dependents syncDiff, syncApply, syncEngine, syncController */
import type {
  LocalFileSystem,
  ManifestData,
  ManifestEntry,
...
export class SyncManifest {
  ...
  /** Loads the manifest from disk, or starts empty when absent or corrupt. @usedBy syncController. @returns SyncManifest */
  static async load(
    fs: LocalFileSystem,
    filePath: string,
    providerId: string,
  ...
  /** Returns the entry for a path, or undefined. @usedBy syncDiff, syncApply. @returns ManifestEntry | undefined */
  get(ns: RemoteNamespace, path: string): ManifestEntry | undefined {
    return this.data.namespaces[ns][path];
  }

  /** Returns all relative paths recorded for a namespace. @usedBy syncDiff. @returns string[] */
  paths(ns: RemoteNamespace): string[] {
    return Object.keys(this.data.namespaces[ns]);
  }

  /** Inserts or replaces the entry for a path. @usedBy syncApply. @returns void */
  set(ns: RemoteNamespace, path: string, entry: ManifestEntry): void {
    this.data.namespaces[ns][path] = entry;
  }

  /** Removes the entry for a path, if present. @usedBy syncApply. @returns void */
  delete(ns: RemoteNamespace, path: string): void {
    delete this.data.namespaces[ns][path];
  }

  /** Persists the manifest to disk. @usedBy syncEngine. @returns void */
  async save(): Promise<void> {
    const json = JSON.stringify(this.data, null, 2);
    await this.fs.writeFile(this.filePath, Buffer.from(json, "utf8"));
  }

  /** Returns a deep-copy snapshot of the underlying data (for tests/inspection). @usedBy tests. @returns ManifestData */
